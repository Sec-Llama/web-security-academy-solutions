...
Companion script for the writeup: 04-bypass-front-end-controls-te-cl.md

What this does:
    Same access-control bypass as the CL.TE version, built on the opposite
    trust discrepancy: the front-end honors Transfer-Encoding and forwards
    the whole chunked body, the back-end honors a short, fixed
    Content-Length and stops right after the chunk-size line, leaving a
    complete smuggled "GET /admin" request (carrying the required
    "Host: localhost") in its buffer. Recovers the delete link for
    "carlos" from the merged admin-panel response, then re-smuggles a
    second TE.CL attack at that exact path to complete the lab.

Usage:
    python 04-bypass-front-end-controls-te-cl.py <lab-url>
    e.g. python 04-bypass-front-end-controls-te-cl.py https://0a1b00fa03d9c8b6803b56b400eb00d5.web-security-academy.net
...
def solve(lab_url: str) -> None:
    ...
    attack2 = build_tecl_payload(host, smuggled2)
