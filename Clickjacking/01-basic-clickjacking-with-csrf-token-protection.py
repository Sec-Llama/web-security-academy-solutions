...
Companion script for the writeup: 01-basic-clickjacking-with-csrf-token-protection.md

What this does:
    Generates the transparent-iframe overlay that frames /my-account and positions
    a "click me" decoy directly on top of the invisible "Delete account" button, then
    delivers it through the lab's own exploit server to the simulated victim. The
    CSRF token on the delete request is never touched -- the victim's own browser,
    own session, and own token fire the real request when they click what they think
    is the decoy.

    The 565px/60px overlay offset was measured directly against the "Delete account"
...
Usage:
    python 01-basic-clickjacking-with-csrf-token-protection.py <lab-url>
    e.g. python 01-basic-clickjacking-with-csrf-token-protection.py https://0a1b00fa03d9c8b6803b56b400eb00d5.web-security-academy.net
...
def solve(lab_url: str) -> None:
    ...
    if "Congratulations" in check.text:
        print("[+] Lab solved -- the victim's own CSRF-protected click deleted their account.")
    else:
